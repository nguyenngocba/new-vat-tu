...
export async function importMaterialsFromExcel(file) {
    try {
        ...
        for (const row of data) {
            try {
                // Lấy tên cột (hỗ trợ nhiều định dạng)
                ...
                if (!name) {
                    errorCount++;
                    errors.push(`Dòng ${data.indexOf(row) + 2}: Thiếu tên vật tư`);
                    continue;
                }
                ...
                if (exists) {
                    errorCount++;
                    errors.push(`Dòng ${data.indexOf(row) + 2}: Vật tư "${name}" đã tồn tại`);
                    continue;
                }
                ...
                state.data.materials.push(newMaterial);
                successCount++;
                
            } catch (err) {
                errorCount++;
                errors.push(`Dòng ${data.indexOf(row) + 2}: ${err.message}`);
            }
        }
        
        if (successCount > 0) {
            saveState();
            addLog('Import Excel', `Đã import ${successCount} vật tư từ Excel`);
        }
        
        return { success: true, count: successCount, errors: errors, total: data.length };
    ...
    try {
        ...
        for (const row of data) {
            try {
                ...
                if (!name) {
                    errorCount++;
                    errors.push(`Dòng ${data.indexOf(row) + 2}: Thiếu tên công trình`);
                    continue;
                }
                ...
                if (exists) {
                    errorCount++;
                    errors.push(`Dòng ${data.indexOf(row) + 2}: Công trình "${name}" đã tồn tại`);
                    continue;
                }
        ...
        if (successCount > 0) {
            saveState();
            addLog('Import Excel', `Đã import ${successCount} công trình từ Excel`);
        }
    ...
    try {
        ...
        for (const row of data) {
            try {
                ...
                if (!name) {
                    errorCount++;
                    errors.push(`Dòng ${data.indexOf(row) + 2}: Thiếu tên nhà cung cấp`);
                    continue;
                }
                ...
                if (exists) {
                    errorCount++;
                    errors.push(`Dòng ${data.indexOf(row) + 2}: Nhà cung cấp "${name}" đã tồn tại`);
                    continue;
                }
        ...
        if (successCount > 0) {
            saveState();
            addLog('Import Excel', `Đã import ${successCount} nhà cung cấp từ Excel`);
        }
...
export function showImportModal(type, onSuccess) {
    ...
    switch(type) {
        case 'materials':
            title = '📥 Import danh sách vật tư từ Excel';
            acceptFormat = 'Tên vật tư, Loại, Đơn vị, Số lượng, Đơn giá, Ngưỡng cảnh báo, Ghi chú';
            break;
        case 'projects':
            title = '📥 Import danh sách công trình từ Excel';
            acceptFormat = 'Tên công trình, Ngân sách';
            break;
        case 'suppliers':
            title = '📥 Import danh sách nhà cung cấp từ Excel';
            acceptFormat = 'Tên nhà cung cấp, SĐT, Email, Địa chỉ';
            break;
        default:
            return;
    }
    ...
    const modalHtml = `
        <div class="modal-hd"><span class="modal-title">${title}</span><button class="xbtn" onclick="closeModal()">✕</button></div>
        <div class="modal-bd">
            <div class="metric-card" style="margin-bottom: 16px; background: var(--accent-bg);">
                <div class="metric-sub">📋 Định dạng file Excel (.xlsx, .xls)</div>
                <div class="metric-sub">Các cột khuyến nghị: ${acceptFormat}</div>
                <div class="metric-sub" style="margin-top: 8px;">
                    <a href="#" id="download-template-${type}" style="color: var(--accent);">📎 Tải file mẫu</a>
                </div>
            </div>
            ...
            <div id="import-progress" style="display: none; margin-top: 12px;">
                ...
                <div id="import-status" class="metric-sub" style="margin-top: 8px;"></div>
            </div>
        </div>
        <div class="modal-ft">
            <button onclick="closeModal()">Hủy</button>
            <button class="primary" id="confirm-import">Import dữ liệu</button>
        </div>
    `;
    ...
    setTimeout(() => {
        ...
        if (confirmBtn) {
            confirmBtn.onclick = async () => {
                ...
                switch(type) {
                    case 'materials':
                        result = await importMaterialsFromExcel(file);
                        break;
                    case 'projects':
                        result = await importProjectsFromExcel(file);
                        break;
                    case 'suppliers':
                        result = await importSuppliersFromExcel(file);
                        break;
                }
                ...
                if (result.success) {
                    ...
                    if (result.errors.length > 0) {
                        statusDiv.innerText += `\n⚠️ Lỗi: ${result.errors.length} dòng`;
...
function downloadTemplate(type) {
    ...
    switch(type) {
        ...
        case 'suppliers':
            data = [
                { 'Tên nhà cung cấp': 'Công ty Thép ABC', 'SĐT': '0912345678', 'Email': 'contact@thepabc.com', 'Địa chỉ': 'Hà Nội' },
                { 'Tên nhà cung cấp': 'Thép XYZ', 'SĐT': '0987654321', 'Email': 'sales@thepxyz.com', 'Địa chỉ': 'TP.HCM' }
            ];
