...
function loadUIConfig() {
    try {
        ...
        if (saved) {
            uiConfig = { ...uiConfig, ...JSON.parse(saved) };
        }
    } catch(e) {}
    applyUIConfig();
}
...
function applyUIConfig() {
    ...
    if (uiConfig.logoType === 'emoji') {
        sidebarLogo.innerHTML = `${uiConfig.logoEmoji} ${uiConfig.appName}`;
    } else if (uiConfig.logoType === 'image' && uiConfig.logoImage) {
        sidebarLogo.innerHTML = `<img src="${uiConfig.logoImage}" style="height: 28px; width: auto; display: inline-block; vertical-align: middle; margin-right: 8px;"> ${uiConfig.appName}`;
    } else {
        sidebarLogo.innerHTML = `🏭 ${uiConfig.appName}`;
    }
}
...
function showEmojiPicker(callback) {
    ...
    showModal(
        `<div class="modal-hd"><span class="modal-title">😊 Chọn Emoji Logo</span><button class="xbtn" onclick="closeModal()">✕</button></div>
        <div class="modal-bd">
            <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 10px; text-align: center;">
                ${emojis.map(emoji => `
                    <div onclick="window.selectEmojiCallback && window.selectEmojiCallback('${emoji}')" style="font-size: 32px; cursor: pointer; padding: 10px; border-radius: 8px; background: var(--surface2);">
                        ${emoji}
                    </div>
                `).join('')}
            </div>
            <div style="margin-top: 15px;">
                <input type="text" id="custom-emoji" placeholder="Hoặc nhập emoji tùy chỉnh" style="width: 100%;">
                <button class="sm primary" id="apply-custom-emoji" style="margin-top: 10px; width: 100%;">Áp dụng</button>
            </div>
        </div>
    ...
    setTimeout(() => {
        ...
        if (applyBtn) {
            applyBtn.onclick = () => {
                ...
                if (customEmoji && callback) callback(customEmoji);
                closeModal();
            };
        }
    }, 100);
    
    window.selectEmojiCallback = (emoji) => {
        if (callback) callback(emoji);
        closeModal();
        delete window.selectEmojiCallback;
    };
}
...
export function bindUISettingsEvents() {
    ...
    if (saveAppName && appNameInput) {
        saveAppName.onclick = () => {
            ...
            if (newName) {
                uiConfig.appName = newName;
                saveUIConfig();
                updateSidebarLogo();
                addLog('Cài đặt', `Đã đổi tên ứng dụng thành: ${newName}`);
                alert('Đã cập nhật tên ứng dụng!');
            }
        };
    }
    ...
    if (selectEmoji) {
        selectEmoji.onclick = () => {
            showEmojiPicker((emoji) => {
                uiConfig.logoType = 'emoji';
                uiConfig.logoEmoji = emoji;
                uiConfig.logoImage = null;
                saveUIConfig();
                updateSidebarLogo();
                const preview = document.getElementById('logo-preview');
                if (preview) preview.innerHTML = `📌 Logo: ${emoji}`;
                addLog('Cài đặt', `Đã chọn emoji logo: ${emoji}`);
            });
        };
    }
    ...
    if (uploadImage) {
        uploadImage.onclick = () => {
            uploadLogoImage((imageData) => {
                uiConfig.logoType = 'image';
                uiConfig.logoImage = imageData;
                uiConfig.logoEmoji = '🏭';
                saveUIConfig();
                updateSidebarLogo();
                const preview = document.getElementById('logo-preview');
                if (preview) preview.innerHTML = '📌 Logo: Ảnh đã tải lên';
                addLog('Cài đặt', 'Đã cập nhật logo ảnh');
    ...
    if (resetLogo) {
        resetLogo.onclick = () => {
            uiConfig.logoType = 'text';
            uiConfig.logoEmoji = '🏭';
            uiConfig.logoImage = null;
            saveUIConfig();
            updateSidebarLogo();
            const preview = document.getElementById('logo-preview');
            if (preview) preview.innerHTML = '📌 Logo: Mặc định (🏭)';
            addLog('Cài đặt', 'Đã đặt lại logo mặc định');
        };
    }
    ...
    if (importBtn) {
        importBtn.onclick = async () => {
            const { showImportBackupModal } = await import('./backup.js');
