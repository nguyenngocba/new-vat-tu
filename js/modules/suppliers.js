...
// ========== HIỂN THỊ CHI TIẾT NHÀ CUNG CẤP (PHÓNG LỚN) ==========
export function showSupplierDetail(supplierId) {
    ...
    const modalContent = `
        ...
        <div class="modal-bd" style="max-height: 70vh; overflow-y: auto;">
            ...
            <div class="tbl-wrap"><table style="min-width: 700px;"><thead><tr><th>Ngày nhập</th><th>Vật tư</th><th>Số lượng</th><th>Đơn giá</th><th>VAT</th><th>Thành tiền</th><th>Ghi chú</th></tr></thead>
            ...
            }).join('') || '<tr><td colspan="7" style="text-align: center;">📭 Chưa có giao dịch nhập hàng nào</td></tr>'}</tbody></table></div>
            
            ${transactions.filter(t => t.invoiceImage).length > 0 ? `
                <div class="sec-title" style="margin-top: 20px;">📎 HÓA ĐƠN ĐÍNH KÈM</div>
                <div class="grid2" style="grid-template-columns: repeat(auto-fill, minmax(200px, 1fr));">
                    ${transactions.filter(t => t.invoiceImage).map(t => `
                        <div class="metric-card" style="cursor: pointer;" onclick="window.open('${t.invoiceImage}', '_blank')">
                            <div class="metric-sub">📄 Ngày: ${t.date}</div>
                            <div class="metric-sub">💰 ${formatMoneyVND(t.totalAmount)}</div>
                        </div>
                    `).join('')}
                </div>
            ` : ''}
...
export function exportSupplierDetail(supplierId) {
    const supplier = supplierById(supplierId);
    if (!supplier) return;
    const transactions = state.data.transactions.filter(t => t.type === 'purchase' && t.supplierId === supplierId).sort((a, b) => new Date(b.date) - new Date(a.date));
    const totalSpent = transactions.reduce((sum, t) => sum + (t.totalAmount || 0), 0);
    ...
    const detailData = transactions.map(t => {
        const mat = state.data.materials.find(m => m.id === t.mid);
        return { 'Ngày nhập': t.date, 'Mã vật tư': t.mid, 'Tên vật tư': mat?.name || 'N/A', 'Số lượng': t.qty, 'Đơn vị': mat?.unit || '', 'Đơn giá (VNĐ)': t.unitPrice, 'VAT (%)': t.vatRate || 0, 'Thành tiền (VNĐ)': t.totalAmount, 'Ghi chú': t.note || '' };
    });
...
export function renderSuppliers() {
  const result = renderSupplierSearchBar() + `<div class="card"><div class="sec-title" style="display: flex; justify-content: space-between; align-items: center;"><span>🏭 DANH SÁCH NHÀ CUNG CẤP</span><button class="sm" onclick="exportAllSuppliersReport()" style="font-size: 11px;">📎 Xuất tất cả báo cáo</button></div><div id="supplier-list-container"></div><div id="supplier-history-modal" style="display:none"></div></div>`;
  setTimeout(() => { bindSupplierSearchEvents(); supplierListContainer = document.getElementById('supplier-list-container'); updateSupplierList(); }, 50);
  return result;
}
...
export function viewSupplierHistory(sid) {
  ...
  showModal(`<div class="modal-hd"><span class="modal-title">📜 Lịch sử nhập hàng - ${escapeHtml(supplier?.name)}</span><button class="xbtn" onclick="closeModal()">✕</button></div>
    ...
    <tbody>${purchaseTxns.map(t => {
        const mat = state.data.materials.find(m => m.id === t.mid);
        return `<tr><td>${t.date}</td><td>${mat?.name || 'N/A'}</td><td>${t.qty} ${mat?.unit || ''}</td><td>${formatMoneyVND(t.unitPrice)}</td><td>${t.vatRate || 0}%</td><td class="text-warning">${formatMoneyVND(t.totalAmount)}</td></tr>`;
    }).join('') || '<tr><td colspan="6">Chưa có giao dịch nào</td></tr>'}</tbody></table></div>
    ${purchaseTxns.filter(t => t.invoiceImage).length > 0 ? `<div class="sec-title" style="margin-top:16px">📎 Hóa đơn đính kèm</div>${purchaseTxns.filter(t => t.invoiceImage).map(t => `<div><a href="${t.invoiceImage}" target="_blank">📄 Xem hóa đơn ngày ${t.date}</a></div>`).join('')}` : ''}
