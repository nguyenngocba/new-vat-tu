import { state, saveState, addLog, showModal, closeModal, genTid, matById, projectById, supplierById, hasPermission } from './state.js';
...
export function savePurchase() {
    ...
    mat.qty += qty;
    mat.cost = Math.round((oldValue + newValue) / mat.qty);
    ...
    addLog('Nhập kho', `${mat.name} - SL: ${qty.toLocaleString('vi-VN')} ${mat.unit} - Giá nhập: ${formatMoneyVND(unitPrice)} - VAT: ${vatRate}% - Tổng: ${formatMoneyVND(totalAmount)} - Nhà cung cấp: ${supplier?.name}`);
