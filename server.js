...
app.post('/api/users-table', async (req, res) => { const u=req.body; await pool.query('INSERT INTO users_table (id,name,username,password,role,permissions) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (id) DO UPDATE SET name=$2,password=$3,role=$4,permissions=$6',[u.id,u.name,u.username,u.password,u.role,u.permissions]); res.json({success:true}); });
